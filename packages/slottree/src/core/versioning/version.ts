...
type OldOf<V> = V extends StorageVersion<infer Old extends AnySchema, AnySchema> ? Old : never;

export type NewOf<V> = V extends { schema: infer New extends AnySchema } ? New : never;
...
type OlderSchemaFor<Tail, Fallback extends AnySchema> =
    Tail extends HCons<infer Older extends StorageVersion, unknown> ? NewOf<Older> : Fallback;

export function hCons<New extends AnySchema, Tail extends HNil | HCons<StorageVersion, unknown>>(
...
export function defineVersionHList<List>(versions: List & AssertVersionHList<List>): List {
    return versions;
}
