...
export function readMainAccount(storage: ReturnType<typeof createWalletStorage>) {
    const portfolio = mustGet(
        getById(storage.read().portfolios, 'portfolio-main'),
        'Portfolio was not migrated'
    );

    return mustGet(getById(portfolio.accounts, 'btc-main'), 'Account was not migrated');
}

export function insertDailyAddress(storage: ReturnType<typeof createWalletStorage>): void {
