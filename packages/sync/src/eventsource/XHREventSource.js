/**
 * PATCHED react-native-sse 1.2.1
 * https://github.com/binaryminds/react-native-sse/blob/97b74627b1b72214c56e0094e1e874653d2c6fb2/src/EventSource.js
 *
 * Patch: skip setting Last-Event-ID header when a custom Last-Event-ID header
 * is already provided via options.headers (lines 96-101).
 * */
...
class EventSource {
  ...
  async open() {
    try {
      ...
      // PATCHED
      const hasCustomLastEventIdHeader = Object.keys(this.headers)
        .map(h => h.toLocaleLowerCase())
        .includes('last-event-id');
      if (this.lastEventId !== null && !hasCustomLastEventIdHeader) {
