...
export type RecordValue<T> =
  NonNullable<T> extends Record<string, infer Value> ? Value : never;
...
declare const noMapMarker: unique symbol;
export type NoMap = typeof noMapMarker;
...
type RecordFromFieldRule<From, Target> =
  NonNullable<Target> extends Record<string, AnyObject>
    ? {
        [K in StringKeyOf<From>]: NonNullable<From[K]> extends Record<
          string,
          AnyObject
        >
...
export type ProjectionShape<From, To> = {
  [K in Extract<RequiredKeys<To>, string>]-?: FieldProjectionRule<
    From,
    To[K],
    K
  >;
} & {
  [K in Extract<OptionalKeys<To>, string>]?: FieldProjectionRule<
    From,
    To[K],
    K
  >;
