...
export function projection<
  ...
  From extends AnyObject = z.output<FromSchema> extends AnyObject
    ? z.output<FromSchema>
    : never,
  To extends AnyObject = z.output<ToSchema> extends AnyObject
    ? z.output<ToSchema>
    : never,
>(
  ...
  const project = ((source: ContainerSlot): ContainerSlot => {
    ...
    const projectedValues = projectShape(
      source,
      shape as Record<string, RuntimeRule>,
    );
