...
describe("wallet storage example", () => {
  it("migrates a deep nested ordered set with projection", () => {
    ...
    const migratedAddress = getById(
      migratedAccount.chains.btc.addresses,
      "addr-0",
    )!;
    ...
    expect(orderedIds(migratedAccount.chains.btc.addresses)).toEqual([
      "addr-0",
    ]);
  });
  ...
  it("writes and reads an inserted ordered-set item", () => {
    ...
    expect(orderedIds(updatedAccount.chains.btc.addresses)).toEqual([
      "addr-0",
      "addr-1",
    ]);
    expect(
      getById(updatedAccount.chains.btc.addresses, "addr-1")?.meta.derivation
        .source.discoveredAtBlock,
    ).toBe(840_000);
    expect(getById(storage.read().portfolios, "portfolio-watch")?.name).toBe(
      "Watch only",
    );
  });
  ...
  it("reorders an ordered-set item", () => {
    ...
    storage.update((draft) => {
      ...
      account.chains.btc.addresses = reorder(
        account.chains.btc.addresses,
        "addr-1",
        0,
      );
    });
  ...
  it("removes an ordered-set item", () => {
    ...
    storage.update((draft) => {
      ...
      account.chains.btc.addresses = remove(
        account.chains.btc.addresses,
        "addr-0",
      );
    });
    ...
    expect(
      getById(prunedAccount.chains.btc.addresses, "addr-0"),
    ).toBeUndefined();
